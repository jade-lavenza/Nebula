...
/// Gets the damage hud colours to use. Can be overridden for extra functionality.
/obj/item/organ/external/proc/get_damage_hud_colours()
	return !BP_IS_PROSTHETIC(src) ? flesh_hud_colours : robot_hud_colours
...
/obj/item/organ/external/proc/get_damage_hud_image()
	...
	var/min_dam_state = min(1,(get_sensed_pain()/max(1,max_damage)))
	if(min_dam_state && dam_state < min_dam_state)
		dam_state = min_dam_state
	// Apply colour and return product.
	var/list/hud_colours = get_damage_hud_colours()
